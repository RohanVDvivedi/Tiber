...
{
	ssize_t ret = tiber_read((long long int)stream_context, data, min(data_size, MAX_IO_SIZE));
	if(ret == -1)
	{
		*error = errno;
		return 0;
...
{
	ssize_t ret = tiber_write((long long int)stream_context, data, min(data_size, MAX_IO_SIZE));
	if(ret == -1)
	{
		*error = errno;
		return 0;
...
{
	if(tiber_close((long long int)stream_context) == -1)
		*error = errno;
}
